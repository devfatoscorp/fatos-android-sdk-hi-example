...
public class RouteListAdapter extends RecyclerView.Adapter<RouteListAdapter.RouteViewHolder>
{
    ...
    public void onBindViewHolder(RouteViewHolder holder, int position)
    {
        ...
        textTimelayoutParams.leftMargin = (int)TypedValue.applyDimension(
        ...
        LinearLayout.LayoutParams textView_KindlayoutParams = (LinearLayout.LayoutParams)textView_Kind
                .getLayoutParams();
        textView_KindlayoutParams.leftMargin = (int)TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 6, mContext.getResources().getDisplayMetrics());
        textView_Kind.setLayoutParams(textView_KindlayoutParams);

        LinearLayout.LayoutParams linearLayout_DistChargelayoutParams = (LinearLayout.LayoutParams)linearLayout_DistCharge
                .getLayoutParams();
        linearLayout_DistChargelayoutParams.leftMargin = (int)TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, 6, mContext.getResources().getDisplayMetrics());
        linearLayout_DistCharge.setLayoutParams(linearLayout_DistChargelayoutParams);

        textDistance.setPadding(0, 0, 0, 0);
        LinearLayout.LayoutParams textDistancelayoutParams = (LinearLayout.LayoutParams)textDistance
                .getLayoutParams();
        textDistancelayoutParams.gravity = Gravity.CENTER_VERTICAL;
        textDistance.setLayoutParams(textDistancelayoutParams);
        textDistance.setTextColor(
                ContextCompat.getColor(mContext, R.color.cardview_onemap_textcolor2));

        LinearLayout.LayoutParams imageChargelayoutParams = (LinearLayout.LayoutParams)imageCharge.getLayoutParams();
        imageChargelayoutParams.topMargin = (int)TypedValue.applyDimension(
        ...
        RelativeLayout.LayoutParams imageView_simulLayoutParams = (RelativeLayout.LayoutParams)imageView_Simul
                .getLayoutParams();
        imageView_simulLayoutParams.width = (int)TypedValue.applyDimension(
        ...
        ivLayoutParams.width = (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50,
        ...
        if(m_gApp.getAppSettingInfo().m_nDistUnit == 0)
        { //0 : km , 1: mile
            textDistance.setText(GUtilLib.getInstance(mContext)
                                         .updateTotalRemainDist(routeCardData.getnLength()));
        }
        else
        {
            textDistance.setText(GUtilLib.getInstance(mContext)
                                         .updateTotalRemainDistForMile(routeCardData.getnLength()));
        }

        //fatostest - 남은시간 일자 제거
        String strTime = GUtilLib.getInstance(mContext)
                                 .updateTotalRemainTimeWithoutDay(routeCardData.getnTime(), false,
                                                                  m_gApp.getFatosLocale());
