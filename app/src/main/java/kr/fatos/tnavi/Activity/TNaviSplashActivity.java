...
//        m_gApp.m_strUUID = strUUIDs;
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
        {
            ...
            int permissionResultACCESS_FINE_LOCATION = checkSelfPermission(
                    Manifest.permission.ACCESS_FINE_LOCATION);
            int permissionResultACCESS_READ_PHONE = checkSelfPermission(
                    Manifest.permission.READ_PHONE_STATE);

            if(permissionResultWRITE_EXTERNAL_STORAGE == PackageManager.PERMISSION_DENIED || permissionResultACCESS_FINE_LOCATION == PackageManager.PERMISSION_DENIED || permissionResultACCESS_READ_PHONE == PackageManager.PERMISSION_DENIED)
            {
            ...
            {
                final TelephonyManager tm = (TelephonyManager)m_Context.getSystemService(
                                           ...
                                           @NonNull int[] grantResults)
    {
        ...
        if(requestCode == 1000)
        {
            for(int i = 0; i < grantResults.length; i++)
            {
                ...
                {
                    Toast.makeText(TNaviSplashActivity.this, "App 실행에 필요한 권한이 설정 되지 않았습니다.",
