...
public class GUtilLib
{
    ...
    public String updateTotalRemainDist(int distance)
    {
        ...
        if(distance >= 1000)
        {
            ...
            if(km >= 100)
            {
                ...
                strResult = String.format("%s %s", strTempKm,
                                          m_Context.getResources().getString(R.string.km));
            }
            else if(km >= 10)
            {
                strResult = String.format("%.1f %s", (float)distance / 1000,
                                          m_Context.getResources().getString(R.string.km));
            }
            else
            {
        ...
        else
        {
            ...
            if(distance >= 10)
            {
                ...
                strResult = String.format("%d %s", distance,
                                          m_Context.getResources().getString(R.string.m));
            }
            else
            {
                strResult = String.format("%d %s", distance,
    ...
    public String updateTotalRemainTime(int secTime, boolean i_bArriveTime, Locale i_lLocale)
    {
        ...
        if(i_bArriveTime)
        {
            ...
            strResult = strTemp.replace("AM", m_Context.getResources().getString(R.string.AM))
                               .replace("PM", m_Context.getResources().getString(R.string.PM));
        }
        else
        {
            int minutes = 0;
            ...
            else
            {
                ...
                if(hour < 24)
                {
                    ...
                    else
                    {
                        strResult = String.format(
                                "%02d " + strHour + " %02d " + m_Context.getString(R.string.min),
                                hour, extraMin);
                    }
                }
                else
                {
                    ...
                    {
                        strResult = String.format("%02d " + strDay + " %02d " + strHour, nDay,
                                                  nExtraHour);
                    }
                    else
                    {
                        strResult = String.format(
                                "%02d " + strDay + " %02d " + strHour + " %02d " + m_Context.getString(
                                        R.string.min), nDay, nExtraHour, extraMin);
                    }
                }
            }
        }
    ...
    //일자 제거된 남은시간
    public String updateTotalRemainTimeWithoutDay(int secTime, boolean i_bArriveTime,
                                                  Locale i_lLocale)
    {
        String strResult = "", strTemp;
        Calendar now = Calendar.getInstance();
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    strResult = String.format(
                            "%02d " + strHour + " %02d " + m_Context.getString(R.string.min), hour,
                            extraMin);
                }
            }
        }
    ...
    public String updateTotalRemainTimeRoute(int secTime, boolean i_bArriveTime, Locale i_lLocale)
    {
        ...
        if(i_bArriveTime)
        {
            ...
            strResult = " " + strTemp.replace("AM", m_Context.getResources().getString(R.string.AM))
                                     .replace("PM",
                                              m_Context.getResources().getString(R.string.PM));
        }
        else
        {
            int minutes = 0;
    ...
    //일자 제거된 남은시간
    public String updateTotalRemainTimeRouteWithoutDay(int secTime, boolean i_bArriveTime,
                                                       Locale i_lLocale)
    {
        String strResult = "", strTemp;
        Calendar now = Calendar.getInstance();
