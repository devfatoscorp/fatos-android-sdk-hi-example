...
public class BlockSnapHelper extends SnapHelper
{
            ...
            throws IllegalStateException
    {
        ...
        if(recyclerView != null)
        {
            ...
            if(layoutManager.canScrollHorizontally())
            {
                ...
                mLayoutDirectionHelper = new LayoutDirectionHelper(
                                      ...
                                      int velocityY)
    {
        ...
        if(velocityX != 0)
        {
            return mLayoutDirectionHelper.getPositionsToMove(lm, mScroller.getFinalX(),
                                                             mItemDimension);
        }

        if(velocityY != 0)
        {
            return mLayoutDirectionHelper.getPositionsToMove(lm, mScroller.getFinalY(),
                                                             mItemDimension);
        }
    ...
    public View findSnapView(RecyclerView.LayoutManager layoutManager)
    {
        ...
        View snapView = (snapPos == RecyclerView.NO_POSITION) ? null : layoutManager.findViewByPosition(
                snapPos);
    ...
    private int calcTargetPosition(LinearLayoutManager layoutManager)
    {
        ...
        else
        {
            ...
            {
                int[] toScroll = mLayoutDirectionHelper.calculateDistanceToScroll(layoutManager,
                                                                                  snapPos);
    ...
    private class LayoutDirectionHelper
    {
        ...
        {
            return (mIsRTL) ? mOrientationHelper.getDecoratedEnd(
                    targetView) - mRecyclerView.getWidth() : mOrientationHelper.getDecoratedStart(
                    targetView);
        }
        ...
        int[] calculateDistanceToScroll(LinearLayoutManager layoutManager, int targetPos)
        {
            ...
            if(layoutManager.canScrollHorizontally())
            {
                ...
                { // scrolling toward top of data
                    if(mIsRTL)
                    {
                        View lastView = layoutManager.findViewByPosition(
                                layoutManager.findLastVisibleItemPosition());
                        out[0] = mOrientationHelper.getDecoratedEnd(
                                lastView) + (firstVisiblePos - targetPos) * mItemDimension;
                    }
                    else
                    {
                        View firstView = layoutManager.findViewByPosition(firstVisiblePos);
                        out[0] = mOrientationHelper.getDecoratedStart(
                                firstView) - (firstVisiblePos - targetPos) * mItemDimension;
                    }
                }
            }
            if(layoutManager.canScrollVertically())
